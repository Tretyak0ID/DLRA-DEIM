...
# K-step (eq. 9): integrate  dK/dt = G(:, J, t) * [V(J,:)]^{-T}
...
function _k_step(Y::LRMat{T}, G, t0::Real, dt::Real,
    ...
    F_K = (K, t) -> begin
        G′  = update_state(G, LRMat(K, Matrix{T}(I, size(K,2), size(K,2)), Y.V))
...
# S-step (eq. 10): integrate  dS̃/dt = -[U1(I,:)]^{-1} * G(I,J,t) * [V0(J,:)]^{-T}
# P_I = ObliqueProjector(U1, k_K): selects I (row indices of Y).
# P_J = ObliqueProjector(V0, k_L): selects J (column indices of Y).
...
function _s_step(U1::Matrix{T}, R1::Matrix{T}, V0::Matrix{T},
    ...
    F_S = (S, t) -> begin
        G′  = update_state(G, LRMat(U1, S, V0))
...
# L-step (eq. 11): integrate  dL/dt = G(I,:,t)^T * [U1(I,:)]^{-T}
...
function _l_step(U1::Matrix{T}, S̃1::Matrix{T}, V0::Matrix{T},
    ...
    F_L = (L, t) -> begin
        G′  = update_state(G, LRMat(U1, L', V0))
...
function _lie_trotter(Y::LRMat, G, t::Real, dt::Real, inner, k_K::Int, k_L::Int)
    ...
    S̃1       = _s_step(U1, R1, Y.V, G, t, dt, inner, P_I, P_J_L)
...
function _strang(Y::LRMat, G, t::Real, dt::Real, inner, k_K::Int, k_L::Int)
    h = dt / 2
    ...
    U2, R2 = _k_step(LRMat(U1, S2, V2), G, t + h, h, inner, P_J2)
...
"""
    ksl_step(Y, G, t, dt, outer, inner, k_K, k_L) -> LRMat
...
"""
function ksl_step(Y::LRMat, G, t::Real, dt::Real,
